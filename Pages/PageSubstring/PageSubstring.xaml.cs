...
namespace Exercise3
{
    public partial class SubstringM : Page
    {
        public SubstringM() { InitializeComponent(); }
        
        private void Exit(object sender, RoutedEventArgs e) { Application.Current.Shutdown(); }

        private void SubstringString(object sender, RoutedEventArgs e)
        {
            int count = -1, index;
            try
            {
                ...
                if (stringCount.Text != "")
                {
                    try
                    {
                        ...
                        if (count < 0 || index > stringOne.Text.Length) { throw new Exception(); }
                    } catch { throw new Exception("Некорректно введено количество символов для удаления"); }
                }
                stringResult.Text = count < 0 ? stringOne.Text.Substring(index) : stringOne.Text.Substring(index, count);
        ...
        private void BackSubstring(object sender, RoutedEventArgs e) { NavigationService.Navigate(new Menu()); }
    }
}